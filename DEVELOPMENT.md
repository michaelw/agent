...
That verifies the staged bundle structure and the installed `brokerctl` payload location, but it is not sufficient for real `SMAppService` integration.
...
5. If you are testing unsigned or relaxed XPC peer checks during broker development, run the local agent with:

```bash
THAND_PRIVILEGE_BROKER_INSECURE_DEV_MODE=1 \
../bin/thand --config ~/tmp/thand-agent/config.yaml agent
```

6. Open the installed app registration manually if needed:

```bash
/Applications/ThandPrivilegeServices.app/Contents/MacOS/ThandPrivilegeServices register
```

7. Approve background items and the daemon in System Settings when macOS prompts for them.

8. Submit and verify a timed sudo request:
