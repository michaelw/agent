...
func (p *BaseProvider) UpsertIdentitiesWithKey(
	identities []Identity,
	keyFunc func(i Identity) []string,
	mergeFunc func(target *Identity, incoming Identity),
) {
	if p.identity == nil {
		logrus.Warningln("provider has no identity support for provider: ", p.GetIdentifier())
		return
	...
	for _, incoming := range identities {
		if existing := findIdentityByKeys(p.identity.identitiesMap, keyFunc(incoming)); existing != nil {
			if mergeFunc != nil {
				mergeFunc(existing, incoming)
			}
			for _, key := range keyFunc(*existing) {
				lowerKey := strings.ToLower(strings.TrimSpace(key))
				if lowerKey == "" {
					continue
				}
				p.identity.identitiesMap[lowerKey] = existing
			}
			updated++
			continue
		}

		p.identity.identities = append(p.identity.identities, incoming)
		identity := &p.identity.identities[len(p.identity.identities)-1]
		for _, key := range keyFunc(*identity) {
			lowerKey := strings.ToLower(strings.TrimSpace(key))
			if lowerKey == "" {
				continue
			}
			p.identity.identitiesMap[lowerKey] = identity
		}
		added++
	}
	...
	logrus.WithFields(logrus.Fields{
		...
		"updated":  updated,
		"total":    totalCount,
	}).Debug("Upserted identities for provider")
...
func findIdentityByKeys(existing map[string]*Identity, keys []string) *Identity {
	for _, key := range keys {
		lowerKey := strings.ToLower(strings.TrimSpace(key))
		if lowerKey == "" {
			continue
		}
		if identity, ok := existing[lowerKey]; ok {
			return identity
		}
	}
