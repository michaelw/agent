...
func TestGrantTimedSudoersPassesInsecureDevModeFlag(t *testing.T) {
	server := &fakeLocalBrokerControlServer{}
	var gotArgs []string
	...
	if !strings.Contains(strings.Join(gotArgs, " "), "--insecure-dev-mode") {
		t.Fatalf("args = %#v, want --insecure-dev-mode", gotArgs)
	}
...
func TestGrantTimedSudoersCodeSigningErrorSuggestsAgentDevMode(t *testing.T) {
	client := &CommandClient{
		executable:      "/test/brokerctl",
		serviceLabel:    "io.thand.agent.privilege-broker",
	...
	errText := err.Error()
	if !strings.Contains(errText, "THAND_PRIVILEGE_BROKER_INSECURE_DEV_MODE=1") {
		t.Fatalf("error = %q, want agent env-var guidance", errText)
	}
	if !strings.Contains(errText, "sign the helper binaries") {
		t.Fatalf("error = %q, want signing guidance", errText)
	}
}
...
func TestNewCommandClientHonorsInsecureDevModeEnvironmentOverride(t *testing.T) {
	previousValue, hadPreviousValue := os.LookupEnv(InsecureDevModeEnvVar)
	if err := os.Setenv(InsecureDevModeEnvVar, "true"); err != nil {
		t.Fatalf("Setenv returned error: %v", err)
	}
	t.Cleanup(func() {
		if hadPreviousValue {
			_ = os.Setenv(InsecureDevModeEnvVar, previousValue)
			return
		}
		_ = os.Unsetenv(InsecureDevModeEnvVar)
	})

	config := &models.BasicConfig{}

	client := NewCommandClient(config)
	if !client.insecureDevMode {
		t.Fatal("expected insecure dev mode to be enabled from environment override")
	}
}
