...
func (p *oauth2Provider) CreateSession(ctx context.Context, authRequest *models.AuthorizeUser) (*models.Session, error) {
	...
	} else if needsUsernameFallback(user, resolved.UsernameClaim) {
		userInfoURL := resolved.UserInfoURL
		if userInfoURL == "" {
			userInfoURL = deriveUserInfoURL(resolved.TokenURL)
		}
...
func needsUsernameFallback(user *models.User, configuredClaim string) bool {
	if user == nil {
		return true
	}
	...
	return len(usernameClaimKeys(configuredClaim)) > 0
}
