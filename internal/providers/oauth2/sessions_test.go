...
func TestCreateSessionUpsertsProviderIdentityWithUsername(t *testing.T) {
	idTokens := []string{
		createTestIDToken(t, map[string]any{
			"sub":            "user-123",
			"email":          "user@example.com",
			"name":           "Example User",
			"email_verified": true,
	...
	tokenServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if call >= len(idTokens) {
			http.Error(w, "too many token requests", http.StatusInternalServerError)
			return
		}
	...
	firstSession, err := provider.CreateSession(context.Background(), &models.AuthorizeUser{
	...
	if firstSession.User.Username != "" {
		t.Fatalf("expected first session username to be empty, got %q", firstSession.User.Username)
	}

	secondSession, err := provider.CreateSession(context.Background(), &models.AuthorizeUser{
	...
	if secondSession.User.Username != "example-user" {
		t.Fatalf("expected second session username to be populated, got %q", secondSession.User.Username)
	}

	identity, err := provider.GetIdentity(context.Background(), "user@example.com")
	if err != nil {
		t.Fatalf("get identity: %v", err)
	}
	if identity.User == nil || identity.User.Username != "example-user" {
		t.Fatalf("expected provider identity cache to be updated with username, got %#v", identity.User)
	}
}

func TestCreateSessionDoesNotRefreshByLabelOnly(t *testing.T) {
	idTokens := []string{
		createTestIDToken(t, map[string]any{
			"sub":            "user-123",
	...
	if _, err := provider.CreateSession(context.Background(), &models.AuthorizeUser{
		Code:        "first-auth-code",
		RedirectUri: "http://localhost/callback",
	}); err != nil {
		t.Fatalf("create first session: %v", err)
	...
	firstIdentity, err := provider.GetIdentity(context.Background(), "first@example.com")
	if err != nil {
		t.Fatalf("get first identity: %v", err)
	}
	if firstIdentity.User == nil || firstIdentity.User.Username != "" {
		t.Fatalf("expected first identity username to remain empty, got %#v", firstIdentity.User)
	}

	secondIdentity, err := provider.GetIdentity(context.Background(), "second@example.com")
	if err != nil {
		t.Fatalf("get second identity: %v", err)
	}
	if secondIdentity.User == nil || secondIdentity.User.Username != "second-user" {
		t.Fatalf("expected second identity username to be populated independently, got %#v", secondIdentity.User)
	}
}
