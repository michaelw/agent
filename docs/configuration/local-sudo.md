...
For low-level unsigned broker development, the agent-side broker client can still be relaxed locally with:

```bash
export THAND_PRIVILEGE_BROKER_INSECURE_DEV_MODE=1
```
...
That insecure mode is intentionally an agent-local setting, not synced provider config.

It is useful for direct-run unsigned helper development, but it is not the primary local integration path anymore. Full local `SMAppService` testing is expected to use the Apple Development-signed install flow above.
...
If `allowed_modes` is omitted, both timed and command mode are allowed.
